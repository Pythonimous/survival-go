...
// Ported from kaya-go/kaya (AGPL-3.0), packages/ai-engine/tests/analysis-utils.test.ts
...
describe("processAnalysis", () => {
  ...
  test("non-finite winRate falls back to tanh approximation", () => {
    ...
    expect(processAnalysis(10, "B", Number.NaN).blackWinRate).toBeCloseTo(expected, 5);
...
describe("getPlayerName", () => {
  test("returns Black with upstream marker", () => {
    expect(getPlayerName("B")).toBe("Black ⚫");
  });

  test("returns White with upstream marker", () => {
