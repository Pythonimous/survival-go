...
// Ported from kaya-go/kaya (AGPL-3.0), packages/ai-engine/src/queue.ts
...
import type { Sign, SignMap } from './goboard';
...
export class AnalysisQueue {
  ...
  private async runOne(req: InternalRequest): Promise<void> {
    ...
    try {
      const result = await this.engine.analyze(req.request.signMap, {
        ...
        maxMctsBatch: req.request.maxMctsBatch,
  ...
  private async runBatch(batch: InternalBatch): Promise<void> {
    ...
    const inputs = stillPending.map(i => ({
      ...
      options: {
        ...
        maxMctsBatch: batch.requests[i].maxMctsBatch,
        includeMove: batch.requests[i].includeMove,
  ...
  private cacheKey(req: AnalysisRequest): string {
    ...
    return JSON.stringify({
      ...
      komi: req.komi,
      history: last5,
      ...(req.includeMove ? { includeMove: req.includeMove } : {}),
    });
  }
