...
// Ported from kaya-go/kaya (AGPL-3.0), packages/ai-engine/src/onnx-session.ts
...
function detectModelProperties(
  ...
  // Dynamic-batch WebGPU models: unbounded batching (e.g. 12×8=96 leaves) exhausts GPU buffers.
  const WEBGPU_DEFAULT_INFERENCE_BATCH = 8;
  if (maxInferenceBatch === Infinity && usedProviders.includes('webgpu')) {
    maxInferenceBatch = WEBGPU_DEFAULT_INFERENCE_BATCH;
  }
...
export async function createOnnxSession(
  ...
  ort.env.wasm.wasmPaths = config.wasmPaths ?? config.wasmPath ?? '/wasm/';
  ort.env.debug = false;
  ...
  assertKayaOutputNames(result.session.outputNames);
