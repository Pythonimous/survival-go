...
const serverStyleAnalysisProvider = vi.hoisted(
  (): AnalysisProvider => ({
    id: "test-server",
    async analyzePosition(input: PositionInput): Promise<AnalysisResult> {
      const response = await fetch(apiUrl(`/api/games/${input.gameId}/analyze`), {
        method: "POST",
    ...
    async getCandidateMoves() {
      throw new Error("getCandidateMoves is not supported in BoardView tests.");
    },
    async requestEngineMove(gameId: string): Promise<EngineMoveResult> {
      ...
      const body = (await response.json()) as {
        survival_score: number;
        metrics: EngineMoveResult["metrics"];
...
vi.mock("@/lib/analysis/selection", () => ({
  getAnalysisProvider: () => serverStyleAnalysisProvider,
}));
...
describe("BoardView", () => {
  ...
  it("shows detailed opening engine progress when the human plays Black", async () => {
    ...
    const requestEngineMoveSpy = vi.spyOn(serverStyleAnalysisProvider, "requestEngineMove");
    requestEngineMoveSpy.mockImplementationOnce(() => openingEngineMoveDeferred);
