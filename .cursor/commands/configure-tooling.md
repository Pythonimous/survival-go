...
1. Ask the developer which tools they want for each category. Offer the current defaults as a reference:
   - Test runner and markers (default: pytest with `unit`, `integration`, `lint`, `e2e`).
   - Linter and formatter (default: flake8 via `pytest -m lint`).
...
- `.cursor/rules/development.mdc` and `.cursor/rules/testing.mdc`: rewrite checklists so they reference the selected tools and remove irrelevant steps.
...
- Any config files tied to removed tooling (for example, delete `mypy.ini` if mypy is not used, add new config files if required).
